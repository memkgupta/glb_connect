...
export const getTracker = async (
  ...
  try {
   ...
   //@ts-ignore
    ...
    const tracker = await Progress.findOne({
      user_id: user._id,
      resource_id: rid,
    });
...
export const startTracker = async (
  ...
  try {
   ...
   //@ts-ignore
    ...
    const isTrackerExists = await Progress.findOne({
      user_id: user._id,
...
export const updateProgress = async (
  ...
  try {
   ...
   //@ts-ignore
    ...
    const { tracker_id, lectureId, taken } = req.body;
    if (!tracker_id || !lectureId || typeof taken !== "boolean") {
      return next(new BadRequestError("Bad request: Missing required fields"));
    }

    // Fetch tracker and related data
...
//@ts-ignore
    ...
    // Update tracker
    if (taken) {
      if (!tracker.taken.includes(lectureId)) {
        tracker.taken.push(lectureId);
      }
      tracker.recent = lectureId;
    } else {
      tracker.taken = tracker.taken.filter(
